...
# def _counterfactuals(family, shap_feats):
#     """Generate simple counterfactual statements based on top feature."""
#     cf = []
#     if shap_feats:
#         top = shap_feats[0]["feature"]
#         if family == "DoS":
#             cf.append({"change": f"If {top} dropped to baseline, threat score would fall to LOW", "outcome": "Low"})
#         elif family in ("R2L", "U2R"):
#             cf.append({"change": f"If {top} was within normal range, classification would shift to MEDIUM", "outcome": "Medium"})
#         elif family == "Probe":
#             cf.append({"change": f"If connection count was under 10, score would fall to LOW", "outcome": "Low"})
#     cf.append({"change": "If activity occurred during business hours, anomaly score would reduce", "outcome": "Medium"})
#     return cf
...
#     # build encoders dict from family_encoder (categoricals were label-encoded during training)
#     # We re-use the same encoder objects saved in artifacts if available
#     encoders = art.get("encoders", {})

#     # 1. Parse raw log → structured events
#     events = parse_log_file(raw_text)
#     if not events:
#         # no parseable events — return a clean bill of health
#         return _empty_result(filename)

#     # 2. Convert to feature matrix
#     X = events_to_matrix(events, feature_cols, encoders)
#     X_scaled = scaler.transform(X)

#     # 3. Predict
#     probs_all  = rf.predict_proba(X_scaled)           # (n_events, n_classes)
#     pred_idxs  = np.argmax(probs_all, axis=1)
#     families   = [class_names[i] for i in pred_idxs]
    
    
#       # ── ADD THIS TEMPORARILY ──────────────────────────────────────
#     for i, (ev, family, probs) in enumerate(zip(events, families, probs_all)):
#         print(f"[DEBUG] Event {i}: hint={ev['meta'].get('event_hint')} "
#               f"→ predicted={family} "
#               f"probs={dict(zip(class_names, [f'{p:.2f}' for p in probs]))}")

#     # 4. SHAP for each event
#     shap_vals = explainer.shap_values(X_scaled)       # list[n_classes] of (n, n_features)
#     shap_arr  = np.array(shap_vals)                   # (n_classes, n_events, n_features)

#     # 5. Global SHAP summary (mean |SHAP| across all events, predicted class)
#     global_shap = np.mean(np.abs(shap_arr), axis=(0, 1))  # (n_features,)
#     top_global  = sorted(zip(feature_cols, global_shap),
#                          key=lambda x: x[1], reverse=True)[:5]
#     max_gs      = top_global[0][1] if top_global else 1.0
#     shap_summary = [{"feature": f, "value": round(float(v), 4), "max_val": round(float(max_gs), 4)}
#                     for f, v in top_global]

#     # 6. Overall threat score = max predicted probability of any attack class
#     attack_mask = [i for i, c in enumerate(class_names) if c != "Normal"]
#     threat_score = float(np.max(probs_all[:, attack_mask]))
#     severity = (
...
#     # 7. Build events list matching _fake_ai_response shape
#     out_events = []
#     now        = datetime.utcnow()

#     for i, (ev, family, pred_idx) in enumerate(zip(events, families, pred_idxs)):
#         meta      = ev["meta"]
#         raw_vals  = X[i]
#         sv_vec    = (shap_arr[pred_idx, i, :]
#                      if shap_arr.ndim == 3 and shap_arr.shape[0] == len(class_names)
#                      else shap_arr[i, :, pred_idx])

#         conf         = float(probs_all[i, pred_idx])
#         status, _    = _SEVERITY_MAP.get(family, ("MEDIUM", 0.5))
#         shap_feats   = _shap_features(sv_vec, feature_cols, raw_vals)
#         cfs          = _counterfactuals(family, shap_feats)
#         source_ip    = meta.get("source_ip") or "unknown"
#         username     = meta.get("username")  or "unknown"

#         verdict = (
#             f"Classified as {family} (confidence {conf:.0%}). "
#             f"Top signal: {shap_feats[0]['feature']} — {shap_feats[0]['reason']}."
#             if shap_feats else f"Classified as {family} with confidence {conf:.0%}."
#         )

#         top_feat = shap_feats[0] if shap_feats else {}
#         neg_feat = next((f["feature"] for f in shap_feats if f["direction"] == "down"),
#                         "logged_in")
...
#             "dest_ip":    "internal",
#             "username":   username,
#             "process":    meta.get("event_hint", "unknown"),
...
#     # 8. Summary sentence
#     attack_types = [e["type"] for e in out_events if e["type"] != "Normal"]
#     summary = (
#         f"Detected {len(attack_types)} threat event(s) in {filename}: "
#         + ", ".join(sorted(set(attack_types)))
...
# ── translate event_hint → human readable attack type ─────────────────────────
...
def _resolve_family(hint: str, pred_family: str,
    ...
    Trusts the model when confident, falls back to hint when uncertain.
    """
    top_prob = float(np.max(probs))

    # model confident — trust it, but fix DoS/R2L brute force confusion
